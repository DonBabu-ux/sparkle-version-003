...
const renderPolls = async (req, res) => {
    try {
        ...
        res.json({ success: true, title: 'Community Polls', initialPolls: polls });
    } catch (error) {
        res.status(500).json({ success: false, title: 'Community Polls', initialPolls: [] });
    }
};
...
const renderPollDetail = async (req, res) => {
    try {
        ...
        res.json({ success: true, title: poll.question, poll });
    } catch (error) {
...
const renderEvents = async (req, res) => {
    try {
        ...
        res.json({ success: true, title: 'Campus Events', initialEvents: events, currentUser: req.user, activeFilter: filter || 'all' });
    } catch (error) {
        console.error('Get Events Data Error:', error);
        res.status(500).json({ success: false, title: 'Campus Events', initialEvents: [], currentUser: req.user, activeFilter: 'all' });
    }
};
...
const renderEventsAdmin = async (req, res) => {
    try {
        ...
        res.json({ success: true, title: 'Manage My Events', initialEvents: events });
    } catch (error) {
        console.error('Events Admin Data Error:', error);
        res.status(500).json({ success: false, message: 'Failed to load managed events' });
    }
};
...
const renderStreams = async (req, res) => {
    try {
        ...
        res.json({ success: true, title: 'Live Streams', initialStreams: streams });
    } catch (error) {
        res.status(500).json({ success: false, title: 'Live Streams', initialStreams: [] });
    }
};
