...
const renderConfessions = async (req, res) => {
    try {
        ...
        logger.error('Get Confessions Data Error:', error);
        res.status(500).json({ success: false, title: 'Community Confessions', confessions: [], user: req.user, filter: 'all', isRandom: false });
    }
};
