...
const renderGroups = async (req, res) => {
    try {
        ...
        logger.error('Get Groups Data Error:', error);
        res.status(500).json({ success: false, error: 'Failed to fetch groups', initialGroups: [], activeFilter: 'all' });
    }
};
...
const createGroupPost = async (req, res) => {
    try {
        ...
        const postId = await Group.createPost(groupId, userId, content, image_url);

        res.status(201).json({ success: true, message: 'Post created', post_id: postId });
    } catch (error) {
...
const getGroupPostsAPI = async (req, res) => {
    try {
        ...
        const posts = await Group.getPosts(groupId, parseInt(limit), parseInt(offset));
        res.json(posts);
    } catch (error) {
        logger.error('Get Group Posts Error:', error);
...
const updateGroupAPI = async (req, res) => {
    try {
        ...
        if (req.files) {
            if (req.files.icon && req.files.icon[0]) updates.icon_url = req.files.icon[0].path;
            if (req.files.cover && req.files.cover[0]) updates.cover_image = req.files.cover[0].path;
        }
...
const getMembersDetailedAPI = async (req, res) => {
    try {
        ...
        logger.error('Get Members Detailed Error:', error);
