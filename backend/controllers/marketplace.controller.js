...
// ========== DATA API ROUTES (Formerly Web Routes) ==========
...
const renderMarketplace = async (req, res) => {
    try {
        ...
        logger.error('Get marketplace data error:', error);
        res.status(500).json({
            success: false,
...
const renderListingDetail = async (req, res) => {
    try {
        ...
        logger.error('Get listing detail error:', error);
        res.status(500).json({
            success: false,
...
const renderSell = async (req, res) => {
    try {
        ...
        logger.error('Get sell page data error:', error);
        res.status(500).json({ success: false, message: 'Failed to load sell page' });
    }
};
...
const renderSellerProfile = async (req, res) => {
    try {
        ...
        if (!profile) {
            return res.status(404).json({ success: false, message: 'This seller does not exist.' });
        }
        ...
        res.status(500).json({ success: false, message: 'Failed to load seller profile' });
    }
};
