...
const renderConnect = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Failed to balance cosmic sparks... Try again later.', error: error.message });
    }
};

module.exports = {
    renderConnect,
    renderSearch: async (req, res) => res.json({ 
        success: true,
        title: 'Search Results', 
        user: req.user, 
        query: req.query.q || '' 
    }),
    renderFollowRequests: async (req, res) => {
        try {
            const requests = await User.getPendingFollowRequests(req.user.userId || req.user.user_id);
            res.json({ 
                success: true,
            ...
            res.status(500).json({ success: false, message: 'Failed to fetch follow requests' });
        }
    },
    blockUser: async (req, res) => res.json({ success: true }),
