    ...
    try {
        ...
        if (media_url && media_url !== 'text') {
            try {
                ...
                const originalName = req.files?.media ? req.files.media[0].originalname : (req.file ? req.file.originalname : 'unknown');
                ...
                const duplicate = await MediaService.findDuplicate(userId, hashChecksum);
                if (!duplicate) {
        ...
        // Invalidate story cache so the dashboard immediately reflects the new story
        try {
            const redisService = require('../services/redis.service');
            await redisService.del(`stories:active:${userId}`);
