...
const searchStart = content.indexOf(marker);
if (searchStart === -1) {
    console.log('❌ Marker not found');
    process.exit(1);
}

// Find the start of the next div or structural element after AnimatePresence
// We want to replace from after the first AnimatePresence to before the second AnimatePresence (which is for comments)
const secondAnimatePresence = content.indexOf('<AnimatePresence>', searchStart + marker.length);
...
// We need to replace everything from after the first </AnimatePresence> up to the start of the next {/* Comments Drawer */}
const commentDrawerMarker = '{/* Comments Drawer (Pink Soft) */}';
const endReplacement = content.indexOf(commentDrawerMarker);

if (endReplacement === -1) {
    console.log('❌ Comment drawer marker not found');
    process.exit(1);
}

const newContent = content.substring(0, searchStart + marker.length) + insertion + content.substring(endReplacement + commentDrawerMarker.length);
fs.writeFileSync(filePath, newContent);
console.log('✅ File repaired successfully with full string replacement');
