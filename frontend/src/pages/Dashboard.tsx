          ...
          <section className="flex flex-col gap-[1.5px] sm:gap-1.5">
            ...
            <div className="bg-white rounded-[8px] sm:rounded-[12px] shadow-sm p-4 animate-fade-in border sm:border border-gray-200">
              ...
              <div className="border-t border-gray-100 pt-2 flex items-center justify-around px-1">
                <button onClick={() => navigate('/moments/create')} className="flex items-center gap-2 px-2 sm:px-3 py-2 hover:bg-gray-100 rounded-lg transition-colors group"><Video size={24} className="text-[#f02849]" /><span className="text-[13px] font-semibold text-gray-500 whitespace-nowrap">Live video</span></button>
                <label className="flex items-center gap-2 px-2 sm:px-3 py-2 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer group"><Image size={24} className="text-[#45bd62]" /><span className="text-[13px] font-semibold text-gray-500 whitespace-nowrap">Photo/video</span><input type="file" multiple className="hidden" accept="image/*" onChange={(e) => { setMediaFiles(prev => [...prev, ...Array.from(e.target.files || [])]); setActiveModal('creationHub'); }} /></label>
